...
const SectionIcon = ({ type }: { type: 'rocket' | 'trophy' | 'calendar' | 'brain' | 'flash' | 'sync' }) => {
  switch (type) {
    case 'rocket':
      return (
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-12 animate-fade-in pb-16">
      ...
      <div className="flex justify-between items-center">
        <button 
          onClick={onReset} 
          className="text-slate-400 hover:text-white flex items-center gap-2 transition-colors text-sm group"
        >
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-8 border border-slate-700 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none transform translate-x-10 -translate-y-10">
          <svg className="w-48 h-48 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zm0 9l2.95 1.47-8.95 4.47L2.95 15.47 12 11zm11 1.53l-2.95 1.47L12 18.53 3.95 14.53 1 16l11 5.5 11-5.5-1.05-1.47z"/></svg>
        </div>
        <div className="flex items-center gap-3 mb-6 relative z-10">
          <div className="p-2 bg-brand-500/10 rounded-xl border border-brand-500/20">
            <SectionIcon type="rocket" />
          </div>
          <h2 className="text-2xl font-bold text-white tracking-tight">The 30-Second Catch-Up</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {report.bigThree.map((item, index) => (
          <div key={index} className="bg-slate-800 rounded-2xl p-6 border border-slate-700 hover:border-yellow-500/40 hover:bg-slate-800/80 transition-all group flex flex-col shadow-lg">
            <div className="flex items-center gap-3 mb-4">
              <span className="flex items-center justify-center w-8 h-8 rounded-full bg-slate-700 text-yellow-500 font-bold text-sm border border-slate-600 group-hover:bg-yellow-500 group-hover:text-slate-900 transition-colors">
                {index + 1}
              </span>
              <div className="h-px bg-slate-700 flex-grow group-hover:bg-slate-600 transition-colors"></div>
            </div>
            <h3 className="font-bold text-lg text-white mb-3 group-hover:text-yellow-400 transition-colors">{item.title}</h3>
      ...
      {report.thenVsNow && report.thenVsNow.length > 0 && (
        <div className="bg-slate-800 rounded-3xl overflow-hidden border border-slate-700 shadow-xl">
          <div className="p-8 pb-4 border-b border-slate-700/50 bg-slate-800/50">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-indigo-500/10 rounded-xl border border-indigo-500/20">
                <SectionIcon type="sync" />
              </div>
              <h2 className="text-2xl font-bold text-white">Then vs. Now</h2>
            </div>
            <p className="text-slate-400 ml-1">How the paradigm has shifted since your last check-in.</p>
          </div>
          
          <div className="grid md:grid-cols-2 gap-px bg-slate-700/50">
             {/* Header Row (Hidden on mobile, visible on desktop if desired, but here we use card style) */}
             
             {report.thenVsNow.map((item, idx) => (
               <React.Fragment key={idx}>
                 {/* Item Container */}
                 <div className="bg-slate-800 p-6 md:col-span-2 grid md:grid-cols-2 gap-6 relative group hover:bg-slate-750 transition-colors">
                    
                    {/* The Old Way */}
                    <div className="space-y-2 relative">
                      <div className="md:hidden text-xs font-bold text-slate-500 uppercase mb-1">The Old Way</div>
                      <div className="flex items-start gap-3">
                         <div className="mt-1.5 w-2 h-2 rounded-full bg-slate-600"></div>
                         <p className="text-slate-400 line-through decoration-slate-600 decoration-2 text-lg">{item.old}</p>
                      </div>
                    </div>

                    {/* Arrow for Desktop */}
                    <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-slate-700 rounded-full items-center justify-center text-slate-400 z-10 border border-slate-600">
                      →
                    </div>

                    {/* The New Way */}
                    <div className="space-y-2">
                       <div className="md:hidden text-xs font-bold text-brand-500 uppercase mb-1 mt-4">The New Way</div>
                       <div className="flex items-start gap-3">
                         <div className="mt-1.5 w-2 h-2 rounded-full bg-brand-500 shadow-[0_0_8px_rgba(14,165,233,0.6)]"></div>
                         <div>
                            <p className="text-white font-bold text-lg">{item.new}</p>
                            <p className="text-brand-400/80 text-sm mt-1 font-medium">{item.benefit}</p>
                         </div>
                      </div>
                    </div>
                 </div>
      ...
      <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700 shadow-xl">
        <div className="flex items-center gap-3 mb-8">
          <div className="p-2 bg-cyan-900/50 rounded-xl border border-cyan-700/50">
             <SectionIcon type="calendar" />
          </div>
          <h2 className="text-2xl font-bold text-white">Timeline of Evolution</h2>
        </div>
        
        <div className="relative border-l-2 border-slate-700 ml-3 md:ml-6 space-y-8 md:space-y-12 pb-4">
          {report.timeline.map((row, idx) => (
            <div key={idx} className="relative pl-8 md:pl-12 group">
              {/* Dot */}
              <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-slate-800 border-2 border-cyan-500 group-hover:bg-cyan-500 group-hover:scale-110 transition-all"></div>
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 md:gap-8">
                <div className="mb-1 md:mb-0 md:w-32 flex-shrink-0">
                  <span className="inline-block py-1 px-2 rounded bg-slate-700 text-cyan-400 text-xs font-mono font-bold">
                    {row.date}
                  </span>
                </div>
                <div className="flex-grow">
                   <h4 className="text-lg font-bold text-white mb-2">{row.event}</h4>
                   <p className="text-slate-400 text-sm leading-relaxed">{row.impact}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* 🧠 Jargon - Conditionally Rendered */}
        {report.terminology.length > 0 && (
          <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700">
             <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-pink-900/50 rounded-xl border border-pink-700/50">
            ...
            <ul className="space-y-5">
              {report.terminology.map((item, idx) => (
                <li key={idx} className="flex gap-4 group">
                   ...
                   <div>
                     <span className="block font-bold text-slate-200 text-lg mb-1">{item.term}</span>
                     <span className="text-sm text-slate-400 leading-relaxed">{item.definition}</span>
                   </div>
                </li>
              ))}
            </ul>
        ...
        {/* ⚡ The New Standard (Replaces Future Outlook) */}
        <div className={`bg-gradient-to-br from-green-900/40 to-slate-800 rounded-3xl p-8 border border-green-700/30 flex flex-col ${report.terminology.length === 0 ? 'md:col-span-2' : ''}`}>
           <div className="flex items-center gap-3 mb-6">
            <div className="p-2 bg-green-900/50 rounded-xl border border-green-500/30">
              <SectionIcon type="flash" />
            </div>
            <h2 className="text-xl font-bold text-white">The New Standard</h2>
          </div>
          <div className="text-slate-300 leading-relaxed flex-grow text-lg space-y-4">
             {/* Render simple paragraphs if content has newlines, or just text */}
      ...
      {report.sources && report.sources.length > 0 && (
        <div className="pt-8 border-t border-slate-800">
           <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-4">Sources & Further Reading</h4>
           <div className="flex flex-wrap gap-2">
             {report.sources.map((source, idx) => (
               <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="text-xs text-brand-400 hover:text-white hover:bg-brand-600 bg-slate-800 px-3 py-2 rounded-lg border border-slate-700 transition-all flex items-center gap-2"
               >
                 <span>🔗</span>
