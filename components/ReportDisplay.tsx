...
const SectionIcon = ({ type }: { type: 'rocket' | 'trophy' | 'calendar' | 'brain' | 'flash' | 'sync' | 'mail' | 'check' }) => {
  switch (type) {
    case 'rocket':
      return (
...
export const ReportDisplay: React.FC<ReportDisplayProps> = ({ report, onReset }) => {
  ...
  const formatReportForEmail = (rep: ParsedReport): string => {
    let text = `RE:SYNC BRIEFING\n================\n\n`;
    text += `Rocket The 30-Second Catch-Up:\n${rep.catchUp}\n\n`;
    
    text += `Trophy The Big Three Milestones:\n`;
    rep.bigThree.forEach((item, i) => {
      text += `${i + 1}. ${item.title}: ${item.description}\n`;
    });
    text += `\n`;

    text += `Then vs. Now:\n`;
    rep.thenVsNow.forEach(item => {
      text += `- Old: ${item.old} -> New: ${item.new} (${item.benefit})\n`;
    });
    text += `\n`;

    text += `Timeline of Evolution:\n`;
    rep.timeline.forEach(row => {
      text += `[${row.date}] ${row.event} - ${row.impact}\n`;
    });
    text += `\n`;

    if (rep.terminology.length > 0) {
      text += `New Terminology:\n`;
      rep.terminology.forEach(item => {
    ...
    text += `Generated by ReSync. Keep Bridging the Gap.`;
    return text;
  };
    ...
    <div className="w-full max-w-4xl mx-auto space-y-12 animate-fade-in pb-16">
      ...
      <div className="flex flex-col md:flex-row md:justify-between items-center gap-4">
        <div className="flex items-center gap-6">
          ...
          >
            <SectionIcon type="mail" />
            Send as Email
          </button>
        </div>
        <span className="text-slate-400 text-xs uppercase tracking-wider font-bold">Generated by Gemini 2.5</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {report.bigThree.map((item, index) => (
          <div key={index} className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-yellow-400 hover:shadow-lg transition-all group flex flex-col shadow-sm">
            ...
            <h3 className="font-bold text-lg text-slate-900 mb-3 group-hover:text-yellow-700 transition-colors">{item.title}</h3>
          ...
          <div className="grid md:grid-cols-2 gap-px bg-slate-100">
             {report.thenVsNow.map((item, idx) => (
               <React.Fragment key={idx}>
                 <div className="bg-white p-6 md:col-span-2 grid md:grid-cols-2 gap-6 relative group hover:bg-slate-50/50 transition-colors">
                    <div className="space-y-2 relative">
                      <div className="md:hidden text-xs font-bold text-slate-400 uppercase mb-1">The Old Way</div>
                    ...
                    <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-white rounded-full items-center justify-center text-slate-400 z-10 border border-slate-200 shadow-sm">
                      →
                    </div>
                    <div className="space-y-2">
                       <div className="md:hidden text-xs font-bold text-brand-600 uppercase mb-1 mt-4">The New Way</div>
                       <div className="flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className={`bg-gradient-to-br from-green-50 to-white rounded-3xl p-8 border border-green-100 flex flex-col shadow-sm ${report.terminology.length === 0 ? 'md:col-span-2' : ''}`}>
           <div className="flex items-center gap-3 mb-6">
      ...
      {isEmailModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md p-8 shadow-2xl transform animate-scale-in">
            {emailStatus === 'sent' ? (
